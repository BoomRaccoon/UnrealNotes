    ...
    \section{Metadata Specifier}
        ...
        \subsection{UPROPERTY}
            ...
            \begin{itemize}
                ...
                \item Transient := it will not be saved on disk and a value that is derived (eg. damagePerSecond = damage/damgeInSeconds)
                \item BlueprintImplementableEvent := so you can define the actual behaviour in the blueprint/LevelBlueprint (it's an event)
                \item BlueprintNativeEvent := designed to be overriden by BP but also has a default native implementation named '[FUNCTIONNAME]\_Implementation'
                \item Meta specifier:
                \begin{itemize}
            ...
            \uline{Make Variable available from BPs}
            \begin{lstlisting}
    UPROPERTY(BlueprintReadWrite)
            \end{lstlisting}
\smallskip
            \uline{Variable can be seen everywhere}
            \begin{lstlisting}
...
\smallskip
    ...
    \section{Adding additional things into automatically added header files}
        \begin{itemize}
            ...
            \item add things you want to be included and useable by other modules ie: \code{\"UMG\", \"EnhancedInput\"}
...
public:
    ...
    \section{Niagara}
        \begin{itemize}
            ...
            \item Objects are
            \begin{itemize}
                \item \code{UNiagaraSystem*}
                \item \code{UNiagaraComponent*}
            \end{itemize}
            \item In order to use Functions use \code{#include NiagaraFunctionLibrary.h}
            \item To add assign the Niagara emitter to the System use \code{ProjectileComp->SetAsset(ProjectileParticles)} 
            \item spawning a NiagaraSystem is made with: \code{UNiagaraFunctionLibrary::SpawnSystemAtLocation(...)}
        \end{itemize}
...
void GetHintAndIcon(FString& Text, UTexture2D*& Texture);
    ...
    \section{Blueprint Library}
        \begin{itemize}
            ...
            \item each function has to be \code{static}
        \end{itemize}
